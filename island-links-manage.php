...
$configFile = __DIR__ . '/island-links-admin-config.php';
$config = file_exists($configFile) ? require $configFile : [];

require_once __DIR__ . '/island-links-admin-guard.php';
...
<body>
  <div class="box">
    ...
    <p class="admin-note">この画面に<strong>合言葉・パスワードによるロックはありません</strong>。URLは運用MENUなどで共有し、ブロックが必要なときは <code>island-links-admin-config.php</code> の <code>allowed_admin_ips</code> またはサーバの Basic 認証（<code>island-links-manage.htaccess.example</code> 参照）を利用してください。</p>
